...
static int parse_unsigned_option(
...
void print_usage(char *program_name) {
  fprintf(stderr,
          ...
          "\n"
          "Options:\n"
          "  --xmin VALUE        real coordinate of the upper-left corner     "
          "(%.17g)\n"
          "  --ymax VALUE        imaginary coordinate of the upper-left corner "
          "(%.17g)\n"
          "  --xmax VALUE        real coordinate of the bottom-right corner   "
          "(%.17g)\n"
          "  --ymin VALUE        imaginary coordinate of the bottom-right "
          "corner (%.17g)\n"
          "  --ppu VALUE         pixels per unit length on the real axis      "
          "(%u)\n"
          "  --dx-factor VALUE   accepted for interface compatibility         "
          "(%u)\n"
          "  --dy-factor VALUE   number of serial horizontal work stripes     "
          "(%u)\n"
          "  --kmax VALUE        maximum Mandelbrot iteration count           "
          "(%u)\n"
          "  --brute VALUE       the threshold for Mariani Silver algorithm   "
          "(%u)\n"
          ...
          program_name, DEFAULT_XMIN, DEFAULT_YMAX, DEFAULT_XMAX, DEFAULT_YMIN,
          DEFAULT_PPU, DEFAULT_DX_FACTOR, DEFAULT_DY_FACTOR, DEFAULT_KMAX,
          DEFAULT_BRUTE, DEFAULT_OUTPUT, program_name);
}
...
int validate_options(options_t *options) {
  ...
  if (!isfinite(width_estimate) || !isfinite(height_estimate) ||
      ...
      width_estimate > (double)UINT_MAX || height_estimate > (double)UINT_MAX) {
...
static int parse_unsigned_option(
  ...
  if (errno != 0 || endptr == argv[*i + 1] || *endptr != '\0' || parsed == 0 ||
      parsed > UINT_MAX) {
