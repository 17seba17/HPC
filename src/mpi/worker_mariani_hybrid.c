...
#pragma omp declare reduction(checksum_add:checksum_t : omp_out.slot[0] +=     \
                                  omp_in.slot[0],                              \
...
void sending_image(unsigned char *local_image, block_t block) {
  ...
  MPI_Send(local_image, size_total, MPI_BYTE, 0, SENDING_IMAGE_TO_MASTER,
           MPI_COMM_WORLD);

  return;
}

void drawAllImage(options_t *options, image_geometry_t *geometry, block_t block,
  ...
  if (image == NULL) {
    printf("Error during image allocation! nel rank %i", rank);
    return;
  }
...
#pragma omp parallel for collapse(2) schedule(dynamic)                         \
  ...
  for (unsigned int row = block.start_y; row < block.end_y; ++row) {
    for (unsigned int col = block.start_x; col < block.end_x; ++col) {
      ...
      size_t pixel_offset =
          ((size_t)(local_r)*width + (size_t)local_c) * 3u + sizeof(block_t);

      colour_from_iteration(iteration, options->kmax, &image[pixel_offset]);

      local_my_checksum =
          my_checksum_update(local_my_checksum, row, col, iteration);
...
void rendering_block(options_t *options, image_geometry_t *geometry,
                     block_t block, render_stats_t *stats,
                     block_t three_blocks[3], unsigned char *image) {

  stats->tiles_processed += 1;

  if ((block.end_x - block.start_x) * (block.end_y - block.start_y) <
      options->brute) {
  ...
  if (upper_left == 0) {
    upper_left =
        checkHorizontalStripe(options, geometry, row, block.start_x, mid_x);
  }
  if (upper_right == 0) {
    upper_right =
        checkHorizontalStripe(options, geometry, row, mid_x, block.end_x);
  }
  ...
  if (right_up == 0) {
    right_up =
        checkVerticalStripe(options, geometry, col, block.start_y, mid_y);
  }

  if (right_down == 0) {
    right_down =
        checkVerticalStripe(options, geometry, col, mid_y, block.end_y);
  }

  if (upper_left + upper_right + down_right + down_left + left_up + left_down +
          right_up + right_down ==
      2 * height + 2 * width) { // if black
  ...
  block_t NW = {
      block.start_x, block.start_y, mid_x, mid_y, upper_left, 0, left_up, 0};

  block_t NE = {mid_x, block.start_y, block.end_x, mid_y, upper_right, 0,
                0,     right_up};

  block_t SE = {mid_x, mid_y,      block.end_x, block.end_y,
                0,     down_right, 0,           right_down};

  block_t SW = {block.start_x, mid_y,     mid_x, block.end_y, 0,
                down_left,     left_down, 0};

  memcpy(&three_blocks[0], &NE, sizeof(block_t));
  memcpy(&three_blocks[1], &SE, sizeof(block_t));
  memcpy(&three_blocks[2], &SW, sizeof(block_t));

  MPI_Send(three_blocks, 3 * sizeof(block_t), MPI_BYTE, 0,
           SENDING_THREE_BLOCKS_TO_MASTER, MPI_COMM_WORLD);
