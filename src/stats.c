...
void print_stats(const image_geometry_t *geometry, const options_t *options,
  ...
  snprintf(buffer, sizeof(buffer),
           ...
           "time working              %.6f s\n"
           "tiles_processed           %u\n\n",
           rank, options->output_path, geometry->width, geometry->height,
           options->kmax, stats->my_checksum.slot[0],
           stats->my_checksum.slot[1], stats->my_checksum.slot[2],
           stats->my_checksum.slot[3], (unsigned long long)stats->inside_pixels,
           inside_fraction, average_iterations,
           (unsigned long long)stats->total_iterations,
           (double)stats->time_waiting / 1000000.0,
           (double)stats->time_working / 1000000.0, stats->tiles_processed);
...
static uint64_t get_delta_us(struct timespec start, struct timespec end) {
  uint64_t start_us =
      (uint64_t)start.tv_sec * 1000000ULL + (start.tv_nsec / 1000ULL);
