  ...
  for (unsigned int block_x = 0; block_x < options->dx_factor; ++block_x) {
    for (unsigned int block_y = 0; block_y < options->dy_factor; ++block_y) {
      ...
      block.start_x = (unsigned int)(((uint64_t)block_x * geometry->width) /
                                     options->dx_factor);
      block.end_x =
          (unsigned int)(((uint64_t)(block_x + 1u) * geometry->width) /
                         options->dx_factor);
      block.start_y = (unsigned int)(((uint64_t)block_y * geometry->height) /
                                     options->dy_factor);
      block.end_y =
          (unsigned int)(((uint64_t)(block_y + 1u) * geometry->height) /
                         options->dy_factor);

      render_stats_t local_stats;
      memset(local_stats.my_checksum.slot, 0,
             sizeof(local_stats.my_checksum.slot));
      local_stats.total_iterations = 0;
      local_stats.inside_pixels = 0;
      local_stats.tiles_processed = 0;
      ...
      tiles_processed += local_stats.tiles_processed;
...
void render_block_brute_force(options_t *options, image_geometry_t *geometry,
  ...
  for (row = block.start_y; row < block.end_y; ++row) {
    ...
    for (col = block.start_x; col < block.end_x; ++col) {
      ...
      pixel_offset = ((size_t)row * (size_t)geometry->width + (size_t)col) * 3u;
...
void render_block(options_t *options, image_geometry_t *geometry, block_t block,
  ...
  unsigned int row_col =
      block.start_y; // single variable for indicating or abscissa or ordinate

  if (upper_left == 0) {
    upper_left =
        checkHorizontalStripe(options, geometry, row_col, block.start_x, mid_x);
  }
  if (upper_right == 0) {
    upper_right =
        checkHorizontalStripe(options, geometry, row_col, mid_x, block.end_x);
  }
  ...
  if (left_up == 0) {
    left_up =
        checkVerticalStripe(options, geometry, row_col, block.start_y, mid_y);
  }
  if (left_down == 0) {
    left_down =
        checkVerticalStripe(options, geometry, row_col, mid_y, block.end_y);
  }
