...
if (idle_workers == NULL) {
    perror("Errore nell'allocazione degli idle workers\n");
...
while (num_idle > 0 && queue_size > 0) {
        ...
        MPI_Send(&block, sizeof(block_t), MPI_BYTE, worker_to_wake, SENDING_BLOCK_TO_WORKER, MPI_COMM_WORLD);
        in_flight++;
    }
    ...
    while(1){
       ...
       if(received_tag==STOPPING_PROCESS){
        ...
        updateTimeWaiting(&stats);}
          ...
          clock_gettime(CLOCK_MONOTONIC, &stats.start_t);
        rendering_block(&options, &geometry,block_to_analyse,&stats, three_block_to_send, image_to_send);
        worked=1;
          clock_gettime(CLOCK_MONOTONIC, &stats.end_t);
