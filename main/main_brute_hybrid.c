...
int main(int argc, char **argv) {
  ...
  unsigned int max_block_width =
      (geometry.width + options.dx_factor - 1) / options.dx_factor;

  unsigned int max_block_height =
      (geometry.height + options.dy_factor - 1) / options.dy_factor;
  ...
  if (rank == 0) {
    ...
    while (queue_size > 0 || in_flight > 0) {
      ...
      MPI_Recv(recv_buffer, incoming_bytes, MPI_BYTE, status.MPI_SOURCE,
    ...
    while (1) {
      ...
      if (received_tag == STOPPING_PROCESS) {
        ...
        clock_gettime(CLOCK_MONOTONIC, &stats.start_t);
        drawAllImage(&options, &geometry, block_to_analyse, &stats,
                     image_to_send);
        worked = 1;
        clock_gettime(CLOCK_MONOTONIC, &stats.end_t);
